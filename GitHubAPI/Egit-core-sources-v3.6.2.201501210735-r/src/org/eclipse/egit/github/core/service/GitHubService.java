...
public abstract class GitHubService {
    ...
    /**
     * function added by Salim
     *
     * Get paged request by performing multiple requests until the limit is reached or no more pages are
     * available or an exception occurs.
     *
     * @param <V>
     * @param iterator
     ...
     * @throws IOException
     */
    protected <V> List<V> getAll(PageIterator<V> iterator, int pagesLimit) throws IOException {
        List<V> elements = new ArrayList<V>();
        int i = 0;
        try {
            while (iterator.hasNext()) {
                ...
                i++;
                if (i == pagesLimit) {
                    System.out.println("GitHubService: getAll:iteratorlimit acceeded:" + pagesLimit);
                    break;
                }
            }
        } catch (NoSuchPageException pageException) {
