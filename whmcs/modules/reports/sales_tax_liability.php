...
if ($calculate) {
	...
	$query = "SELECT COUNT(*),SUM(total),SUM(tblinvoices.credit),SUM(tax),SUM(tax2) FROM tblinvoices INNER JOIN tblclients ON tblclients.id=tblinvoices.userid WHERE datepaid>='$startdate' AND datepaid<='$enddate' AND tblinvoices.status='Paid' AND currency='$currencyid'";
	$result = mysql_query($query);
	$data = mysql_fetch_array($result);
	$numinvoices = $data[0];
	$total = $data[1] + $data[2];
