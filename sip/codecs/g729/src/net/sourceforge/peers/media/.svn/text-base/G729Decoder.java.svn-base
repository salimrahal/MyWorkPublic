...
import net.sourceforge.peers.media.g729.DecLD8K;
//import net.sourceforge.peers.media.g729.Frame;
import net.sourceforge.peers.media.g729.IntegerPointer;
import net.sourceforge.peers.media.g729.LD8KConstants;
import net.sourceforge.peers.media.g729.PParity;
import net.sourceforge.peers.media.g729.PostFil;
import net.sourceforge.peers.media.g729.PostPro;
...
public class G729Decoder extends Decoder {
	...
	public byte[] process(byte[] data)
	{
		...
		//se carga el input frame
		input = Memory.allocate(data.length);
		input.setLength(data.length);
		inputBytes = input.getData();
		for (int q = 0; q < data.length; q++) {
            inputBytes[q] = data[q];
        }
		
        circular.addData(inputBytes);
        ...
        if (speechWindow != null) {
            ...
            res = Memory.allocate(input.getLength());
            res.setLength(0);
