...
public class BuddyCellRenderer extends JPanel implements ListCellRenderer {
    ...
    private Icon redStatusIcon;//to cover: the busy: supposed to be on call
    private Icon greenStatusIcon;//avalilable status
    private Icon yellowStatusIcon;//to cover: brb, dnd, outtolusch
    private Icon whiteStatusIcon;//to cover: Offline
	...
	switch (adapter.getStatus()) {
	    ...
	    case ONLINE: statusLabel.setIcon(greenStatusIcon); break;
                //red status
            case BUSY: statusLabel.setIcon(redStatusIcon); break;
            case ONTHEPHONE: statusLabel.setIcon(redStatusIcon); break;
                //yellow status
            case AWAY: statusLabel.setIcon(yellowStatusIcon); break;
            case BERIGHTBACK: statusLabel.setIcon(yellowStatusIcon); break;
            case OUTTOLUNCH: statusLabel.setIcon(yellowStatusIcon); break;
	    case IDLE: statusLabel.setIcon(yellowStatusIcon); break;  
            //white icon status
            case OFFLINE: statusLabel.setIcon(whiteStatusIcon); break;     
            //the OFFLINE or UNKNOWN are handled here: (no image status displayed)
...
//	else {
	...
	Icon buddyIcon = (adapter.getStatus() == Adapter.Status.OFFLINE) ?
	    offlineBuddyIcon(adapter.getBuddyIcon()) :
	...
	return new ImageIcon(getClass().getResource("/buddyListplugin/resources/" + name));
    }
	...
	blankStatusIcon = new BlankIcon(greenStatusIcon);
        //load yellow icon
        yellowStatusIcon = loadIcon("yellow-led.png");
         //load white icon
        whiteStatusIcon = loadIcon("white-led.png");
    ...
    public static class Adapter {
        //equivalent to MSNstatus in presentity XML
	public enum Status {ONLINE, OFFLINE, AWAY,UNKNOWN, IDLE, BUSY, BERIGHTBACK, ONTHEPHONE, OUTTOLUNCH };
         //equivalent to Address-status in presentity XML
        public enum Status_Address {OPEN, CLOSED, INUSE};
