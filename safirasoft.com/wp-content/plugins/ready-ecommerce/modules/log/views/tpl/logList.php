...
// <!--
var toeAllLogData = <?php echo utils::jsonEncode($this->logs)?>;
...
<div id="toeLogTabs">
    <ul>
        <?php foreach($this->logTypes as $type => $tInfo) {?>
            <li><a href="#toeLogs<?php echo $type?>"><?php lang::_e($tInfo['label'])?></a></li>
        <?php }?>
    </ul>
    <?php foreach($this->logTypes as $type => $tInfo) {?>
    <div id="toeLogs<?php echo $type?>" class="toeLogTab">
        <table width="100%">
            <tr class="toe_admin_row_header">
                <td><?php lang::_e('Log ID')?></td>
                <td><?php lang::_e('Log Date')?></td>
            </tr>
		<?php if(!empty($this->logs[$type])) {?>
			<?php foreach($this->logs[$type] as $l) { ?>
            <tr class="toe_admin_row toeLogRow">
                <td><?php echo $l['id']?></td>
                <td><?php echo date(S_DATE_FORMAT_HIS, $l['date_created'])?></td>
            </tr>
			<?php } ?>
		<?php }?>
        </table>
